...
const _Filter = ({
  classes, selectedVenueType, onTypeCheckboxSelect, onScaleCheckboxSelect, onFilterClear
}) => (
  <div className={classes.root}>
    ...
    <div>
      { scale.map(s => (
        <label className={classes.checkboxContainer} htmlFor={s} key={`checkbox-${s}`}>
          <input
            id={s}
      ...
      { types.map(type => (
        <label className={classes.checkboxContainer} htmlFor={type} key={`checkbox-${type}`}>
          <input
            className={classes.checkbox}
            type="checkbox"
            name={type}
    ...
    <div className={classes.clearContainer} onClick={onFilterClear} tabIndex={0} onKeyPress={onFilterClear} role="button">
